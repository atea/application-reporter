...
task DeployResources -depends ResourceGroup,StorageAccount,AppServicePlan,FunctionsApp
...
Task FunctionsApp -depends ResourceGroup,AppServicePlan{
...
Task DeployCode -depends FunctionsApp{
    ...
    Compress-Archive -Path (Get-ChildItem "$PSScriptRoot\AzFunctions").FullName -DestinationPath $TestFile -Force
